...
contract SPELLETHStrategy is ERC20, AccessControl, IStrategy {
  ...
  constructor( string memory _name,
    ...
    depositToken = IPair(0x8f93Eaae544e8f5EB077A1e09C1554067d9e2CA8); // BLP
    rewardToken1 = IERC20(0xd4d42F0b6DEF4CE0383636770eF773390d85c61A); // BAL
    //rewardToken2 = IERC20(0x3E6648C5a70A150A88bCE65F4aD4d506Fe15d2AF);
    stakingContract = IMiniChefV2(0xF4d73326C13a4Fc5FD7A064217e12780e9Bd62c3); // Still use?
    router = IRouter(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506); // not needed
    pid = 11; // PoolID on Balancer is address
  ...
  /**
   * @notice set destination for admin fees generated by this pool
  ...
  // @TODO: tokenAddress=<someTokenAddress>;
  // pendingBAL = gauge.claimable_rewards(userAddress).call();
  ...
  function reinvest() external override onlyEOA {
    ...
    require(unclaimedRewards >= MIN_TOKENS_TO_REINVEST, "MIN_TOKENS_TO_REINVEST");
     if (address(arbi) != 0x0000000000000000000000000000000000000000) {
        require(arbi.balanceOf(msg.sender) >= 69000000000000000000, "insufficient ARBI balance");
    }
    _reinvest(unclaimedRewards);
  }
  ...
  function _convertRewardTokensToDepositTokens(uint amount) private returns (uint) {
    ...
    // do this first b/c code below also generates more SPELL
   // (uint256 amountOutSPELL, uint256 amountOutWETH) = convertSpell();

    // swap reward1 to token0(USDC)
    ...
    router.swapExactTokensForTokens(amountIn, amountOutToken0, path0, address(this), block.timestamp);
       ...
       // @TODO: batchSwap() or swapExactAmountOut() 
